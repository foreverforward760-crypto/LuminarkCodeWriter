...
class TestSAPGeometryClinicalTrials:
    ...
    def test_all_centroids_self_classify(self):
        ...
        for s, centroid in STAGE_CENTROIDS.items():
            ...
            assert classified == s, (
                f"Stage {s} centroid classified as {classified}"
            )
...
class TestSAPEnergyClinicalTrials:
    ...
    def test_trap_energy_always_in_unit_interval(self):
        ...
        for x in test_vectors:
            for stage in range(10):
                ...
                assert 0.0 <= e <= 1.0, \
                    f"trap_energy(stage={stage}, x={x}) = {e} out of [0,1]"
    ...
    def test_zero_vector_low_trap_energy(self):
        ...
        for stage in [3, 5, 7, 8]:
            ...
            assert e < 0.9, f"Stage {stage} trap energy {e} unexpectedly high for low-tension vector"
...
class TestBayesianClinicalTrials:
    ...
    def test_posterior_sums_to_one(self, bayesian):
        ...
        for x in test_vectors:
            ...
            assert abs(post.sum() - 1.0) < 1e-5, \
                f"Posterior sum {post.sum()} ≠ 1.0 for x={x}"
    ...
    def test_geometric_mask_blocks_forbidden_transition(self, bayesian):
        ...
        for s in range(5):
            assert post[s] < 1e-9, \
                f"Stage {s} should be masked after Stage 5, got P={post[s]:.6f}"
    ...
    def test_forward_returns_all_keys(self, bayesian):
        ...
        result   = bayesian.forward(x)
        required = {"posterior", "dominant_stage", "expected_stage", "entropy",
                    "trap_energy", "energy_gradient", "arc", "geometric_valid", "stage_metadata"}
...
class TestLyapunovClinicalTrials:
    ...
    def test_dV_negative_when_energy_decreases(self, lyapunov):
        ...
        dv = lyapunov.dV(
            ...
            entropy_after=1.5,  energy_after=0.45, velocity_after=0.3,
        )
    ...
    def test_vulnerability_scanner_detects_instability(self):
        ...
        assert result["instability_count"] > 0, \
            "Scanner failed to detect injected instability"
...
class TestSAPPsychiatristClinicalTrials:
    ...
    def test_assertion_error_maps_to_stage_3(self, psychiatrist):
        ...
        assert d.stage in (3, 8), \
            f"AssertionError should map to Stage 3 or 8, got {d.stage}"
    ...
    def test_recursion_error_maps_to_stage_5(self, psychiatrist):
        ...
        assert d.stage in (5, 9), \
            f"RecursionError should map to Stage 5 or 9, got {d.stage}"
